...
import type { BoutResult, BoutLogEntry, Rikishi } from "@/engine/types";
import { Play, Pause, RotateCcw, SkipForward, FastForward, Volume2, VolumeX } from "lucide-react";
...
export function BoutReplayViewer({
  ...
  const [eastPos, setEastPos] = useState<RikishiPosition>({
    x: -30, y: 0, rotation: 0, scale: 1, state: "standing"
  });
  const [westPos, setWestPos] = useState<RikishiPosition>({
    x: 30, y: 0, rotation: 180, scale: 1, state: "standing"
  });
  
  const animationRef = useRef<number>();
  const lastTickRef = useRef<number>(0);
  
  // Get current log entry for narration
  const currentNarration = currentLogIndex >= 0 && currentLogIndex < result.log.length
    ? result.log[currentLogIndex].description
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      setPhaseProgress(prev => {
        ...
        const newProgress = prev + (delta / phaseDuration) * 100;
      ...
      // Update positions based on phase progress
      updatePositionsDuringPhase(currentPhase, phaseProgress);
  ...
  }, [isPlaying, currentPhase, playbackSpeed]);
  
  const updatePositionsForPhase = (phase: ReplayPhase) => {
  ...
  const updatePositionsDuringPhase = (phase: ReplayPhase, progress: number) => {
    // Subtle animation during phases
  ...
  const advanceLogIndex = (phase: ReplayPhase) => {
    ...
    const nextIndex = result.log.findIndex((entry, i) => 
      i > currentLogIndex && entry.phase === targetPhase
    );
  ...
  const loserName = result.winner === "east" ? westRikishi.shikona : eastRikishi.shikona;
          ...
          <div className="flex items-center gap-2">
            ...
            {currentPhase === "complete" && (
              <Badge variant="default">
                {winnerName} wins by {result.kimariteName}
              </Badge>
            )}
          </div>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {currentPhase === "complete" && (
          ...
          <p className="text-xl font-display font-bold">{result.kimariteName}</p>
            {result.upset && (
              <Badge variant="destructive" className="mt-2">UPSET!</Badge>
            )}
          </div>
        )}
      </CardContent>
  ...
  return (
    <div className={cn(
      ...
      {(state === "grappling" || state === "pushing") && (
        <>
          <div className={cn(
            "absolute top-4 -left-3 w-4 h-2 rounded-full",
            baseColor
          )} />
          <div className={cn(
            "absolute top-4 -right-3 w-4 h-2 rounded-full",
            baseColor
          )} />
        </>
      )}
      ...
      {/* Victory crown */}
      {isWinner && state === "victory" && (
        <div className="absolute -top-6 text-yellow-500">👑</div>
      )}
    </div>
  );
}
