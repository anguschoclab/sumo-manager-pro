...
function computeCareerStats(rikishi: Rikishi) {
  ...
  const totalBouts = rikishi.careerWins + rikishi.careerLosses;
...
export default function AlmanacPage() {
  ...
  // Get all rikishi sorted by rank tier
  ...
  const filteredRikishi = useMemo(() => {
    ...
    return allRikishi.filter(r => 
      r.shikona.toLowerCase().includes(query) ||
  ...
  const filteredHeya = useMemo(() => {
    ...
    return allHeya.filter(h => 
      ...
      (h.nameJa && h.nameJa.includes(searchQuery))
    );
  }, [allHeya, searchQuery]);

  // Top champions by career wins
  ...
  const yushoWinners = useMemo(() => {
    ...
    for (const record of world.history) {
      if (record.yusho) {
        counts.set(record.yusho, (counts.get(record.yusho) || 0) + 1);
      }
    }
    return Array.from(counts.entries())
      .map(([id, count]) => ({ rikishi: getRikishi(id), count }))
      .filter(e => e.rikishi)
      .sort((a, b) => b.count - a.count);
  }, [world.history, getRikishi]);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-2xl font-bold">
                    {allRikishi.filter(r => r.rank === "yokozuna").length}
                  </div>
          ...
          <TabsContent value="rikishi">
            <Card className="paper">
              <CardHeader>
                ...
                <CardDescription>
                  {filteredRikishi.length} wrestlers • Click to view profile
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      return (
                        ...
                        >
                          <div className={`w-1 h-12 rounded-full ${
                            rikishi.side === "east" ? "bg-east" : "bg-west"
                          }`} />
          ...
          <TabsContent value="heya">
            <Card className="paper">
              <CardHeader>
                ...
                <CardDescription>
                  {filteredHeya.length} active stables • Click to view details
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ScrollArea className="h-[500px]">
                  <div className="space-y-2">
                    {filteredHeya.map(heya => {
                      const rosterCount = heya.rikishiIds.length;
                      const sekitoriCount = heya.rikishiIds
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <Card className="paper">
                <CardHeader>
                  ...
                  <CardDescription>
                    Championship victories in recorded history
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
            ...
            <Card className="paper">
              ...
              <CardContent>
                <div className="space-y-4">
                  {/* Best Win Rate */}
                  {allRikishi.length > 0 && (() => {
                    const bestWinRate = [...allRikishi]
                      .filter(r => r.careerWins + r.careerLosses >= 30)
                      .sort((a, b) => {
                        const rateA = a.careerWins / (a.careerWins + a.careerLosses);
                        const rateB = b.careerWins / (b.careerWins + b.careerLosses);
                        return rateB - rateA;
                      })[0];
                    
                    if (!bestWinRate) return null;
                    
                    const rate = (bestWinRate.careerWins / (bestWinRate.careerWins + bestWinRate.careerLosses) * 100).toFixed(1);
                    
                    return (
                      <div className="p-4 rounded-lg bg-secondary/50">
                        <div className="text-sm text-muted-foreground">Best Career Win Rate (min 30 bouts)</div>
                        <Link to={`/rikishi/${bestWinRate.id}`} className="flex items-center gap-2 mt-1 hover:text-primary">
                          <span className="font-display font-bold text-lg">{bestWinRate.shikona}</span>
                  ...
                  {/* Most Experienced */}
                  {allRikishi.length > 0 && (() => {
                    const mostBouts = [...allRikishi].sort((a, b) => 
                      (b.careerWins + b.careerLosses) - (a.careerWins + a.careerLosses)
                    )[0];
                    
                    return (
                      <div className="p-4 rounded-lg bg-secondary/50">
                        <div className="text-sm text-muted-foreground">Most Career Bouts</div>
                        <Link to={`/rikishi/${mostBouts.id}`} className="flex items-center gap-2 mt-1 hover:text-primary">
                          <span className="font-display font-bold text-lg">{mostBouts.shikona}</span>
                          <span className="font-mono">{mostBouts.careerWins + mostBouts.careerLosses} bouts</span>
                        </Link>
                      </div>
                    );
                  })()}
                  ...
                  {/* Largest Stable */}
                  {allHeya.length > 0 && (() => {
                    const largest = [...allHeya].sort((a, b) => b.rikishiIds.length - a.rikishiIds.length)[0];
                    ...
                    return (
                      <div className="p-4 rounded-lg bg-secondary/50">
                        ...
                        <Link to={`/stable/${largest.id}`} className="flex items-center gap-2 mt-1 hover:text-primary">
                          ...
                          <span className="font-mono">{largest.rikishiIds.length} wrestlers</span>
                        </Link>
                      </div>
                    );
                  })()}
