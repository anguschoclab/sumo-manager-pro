 ...
 import type { Rikishi, Rank, RikishiStats, TacticalArchetype, WorldState } from "./types";
 ...
 export function checkRetirement(rikishi: Rikishi, currentYear: number): string | null {
   ...
   // Create deterministic RNG based on rikishi id and year
   const rng = rngFromSeed(`retirement-${rikishi.id}-${currentYear}`, "lifecycle", "retirement");
 ...
 export function generateRookie(world: WorldState, currentYear: number, targetRank: Rank = "jonokuchi"): Rikishi {
   ...
   const rng = rngFromSeed(`${world.seed}-rookie-${rookieId}`, "lifecycle", "rookie");
